...
class GithubAccessService:
    ...
    def handle_oauth_callback(self, user_id: str, code: str) -> Dict[str, Any]:
        ...
        self._sync_collaborators_for_user(user_id, access, triggered_by="oauth_callback")
    ...
    def disconnect_github(self, user_id: str) -> None:
        ...
        for pkg in packages:
            self._github.remove_collaborator(pkg.github_owner, pkg.github_repo, access.github_username)
            self._log_repo.log(user_id, str(pkg.id), SyncAction.REMOVE_COLLABORATOR, "manual")
    ...
    def on_subscription_activated(self, user_id: str, plan_id: str) -> None:
        ...
        if not pkg:
            return
        self._github.add_collaborator(pkg.github_owner, pkg.github_repo, access.github_username, pkg.github_protected_branch)
        access.access_status = AccessStatus.ACTIVE
        access.grace_expires_at = None
        self._access_repo.save(access)
        self._log_repo.log(user_id, str(pkg.id), SyncAction.ADD_COLLABORATOR, "subscription_event")

    def on_subscription_cancelled(self, user_id: str, plan_id: str, trailing_days: int = 0) -> None:
    ...
    def on_subscription_payment_failed(self, user_id: str, plan_id: str, trailing_days: int = 0) -> None:
    ...
    def on_subscription_renewed(self, user_id: str, plan_id: str) -> None:
        ...
        if access.deploy_token:
            try:
                ...
                pass
        new_token = self._github.create_deploy_token(pkg.github_owner, pkg.github_repo, access.github_username)
        access.deploy_token = new_token  # TODO: encrypt
        access.access_status = AccessStatus.ACTIVE
        access.grace_expires_at = None
        self._access_repo.save(access)
        self._log_repo.log(user_id, str(pkg.id), SyncAction.TOKEN_ROTATED, "subscription_event")
    ...
    def revoke_expired_grace_access(self) -> int:
        ...
        for access in expired:
            ...
            for pkg in packages:
                ...
                if access.deploy_token:
                    self._github.revoke_deploy_token(access.deploy_token)
                self._log_repo.log(str(access.user_id), str(pkg.id), SyncAction.REMOVE_COLLABORATOR, "scheduler")
    ...
    def _sync_collaborators_for_user(self, user_id: str, access: GhrmUserGithubAccess, triggered_by: str) -> None:
        """Add user as collaborator for all their active subscribed packages."""
        packages = self._get_packages_for_user(user_id)
        for pkg in packages:
            try:
                self._github.add_collaborator(pkg.github_owner, pkg.github_repo, access.github_username, pkg.github_protected_branch)
                self._log_repo.log(user_id, str(pkg.id), SyncAction.ADD_COLLABORATOR, triggered_by)
            except Exception:
                pass
