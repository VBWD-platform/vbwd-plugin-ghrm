...
class GhrmAccessLog(BaseModel):
    ...
    user_id = db.Column(db.UUID, db.ForeignKey("user.id", ondelete="SET NULL"), nullable=True, index=True)
    package_id = db.Column(db.UUID, db.ForeignKey("ghrm_software_package.id", ondelete="SET NULL"), nullable=True, index=True)
    action = db.Column(db.String(32), nullable=False)
    triggered_by = db.Column(db.String(64), nullable=False)  # subscription_event | manual | scheduler | sync
