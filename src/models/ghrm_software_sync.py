...
class GhrmSoftwareSync(BaseModel):
    ...
    software_package_id = db.Column(db.UUID, db.ForeignKey("ghrm_software_package.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
    ...
    cached_releases = db.Column(db.JSON, nullable=True, default=list)   # [{tag, date, notes, assets:[{name,url}]}]
    cached_screenshots = db.Column(db.JSON, nullable=True, default=list)  # [{url, caption}]
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "latest_released_at": self.latest_released_at.isoformat() if self.latest_released_at else None,
